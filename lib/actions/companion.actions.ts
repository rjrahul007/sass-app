...
export const getAllCompanions = async ({
  ...
  if (!data) throw new Error("Failed to fetch companions");
  return data;
};
...
export const addToSessionHistory = async (companionId: string) => {
  ...
  if (error)
    throw new Error(error.message || "Failed to update session history");
  return data;
};
...
export const getRecentSessions = async (limit = 10) => {
  ...
  const { data, error } = await supabase
    .from("sessions_history")
    .select("companions:companion_id(id, name, subject, topic, duration)")
  ...
  const unique = Array.from(new Map(companions.map((c) => [c.id, c])).values());
